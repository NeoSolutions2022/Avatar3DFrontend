...
function runtimeAssetUrl(value, runtimeBase, manifest) {
  ...
  url.searchParams.set("build", manifest.builtAtUtc || "20260822-clean2");
  return url.href;
}
...
async function initializeAvatar(avatarId, resumePose = state.activePose) {
  ...
  const instance = await createUnityInstance(
    elements.canvas,
    {
      ...
      productVersion: "2026.08.22-clean-retarget.2",
  ...
  elements.loader.classList.add("hidden");
  emitStatus("ready");
  postToParent("neotalk:ready", { version: "2026.08.22-clean-retarget.2", capabilities: ["sign", "avatar", "zoom", "loop", "background", "playback"] });
