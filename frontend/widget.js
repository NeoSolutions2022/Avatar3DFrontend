...
function runtimeAssetUrl(value, runtimeBase, manifest) {
  ...
  url.searchParams.set("build", manifest.builtAtUtc || "20260822-clean3");
  return url.href;
}
...
async function initializeAvatar(avatarId, resumePose = state.activePose) {
  ...
  const instance = await createUnityInstance(
    elements.canvas,
    {
      ...
      productVersion: "2026.08.22-clean-retarget.3",
  ...
  elements.loader.classList.add("hidden");
  emitStatus("ready");
  postToParent("neotalk:ready", { version: "2026.08.22-clean-retarget.3", capabilities: ["sign", "avatar", "zoom", "loop", "background", "playback"] });
